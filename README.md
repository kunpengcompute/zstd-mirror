...
- [2026.06.30]：发布基于开源Zstd 1.5.7优化的鲲鹏ZSTD，覆盖块压缩解压和流压缩解压优化路径。
...
本仓库是面向鲲鹏平台的Zstd优化仓，基于上游Zstd 1.5.7进行性能优化，重点覆盖块压缩、块解压、流压缩、流解压等常用路径。优化不改变用户侧接口，应用仍使用开源Zstd公开C接口完成开发和集成。
...
针对鲲鹏优化的Zstd的每个发布版本特性变更详细信息，请参见《[版本说明书](docs/zh/release_notes.md)》。

## 环境部署

针对鲲鹏优化的Zstd编译环境、依赖获取与安装步骤参见《[安装指南](docs/zh/installation_guide.md)》。

## 快速入门

针对鲲鹏优化的Zstd的安装步骤和快速使用Zstd的命令行与C接口指导请参见《[快速入门](docs/zh/quick_start.md)》。

## 文档

| 资源名称 | 资源简介 |
| -- | -- |
| [版本说明书](docs/zh/release_notes.md) | 提供针对鲲鹏优化的Zstd版本基础信息和特性更新说明。 |
| [快速入门](docs/zh/quick_start.md) | 提供针对鲲鹏优化的Zstd命令行与C API快速上手示例。 |
| [API参考](docs/zh/api_reference.md) | 提供Zstd常见公开接口说明与函数定义。 |
| [安装指南](docs/zh/installation_guide.md) | 提供针对鲲鹏优化代码Zstd的代码获取、编译和安装指导。 |
