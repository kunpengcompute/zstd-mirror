...
执行本文档操作前确保已安装基于鲲鹏优化的Zstd，详细安装步骤请参见《[安装指南](./installation_guide.md)》。
...
以下示例演示如何使用Zstd公开C API完成一次性内存块压缩和解压。
...
int main(void)
{
    ...
    size_t decompressedSize = ZSTD_decompress(decompressed, (size_t)contentSize,
...
将示例保存为`quick_start.c`后，使用如下命令编译和运行。
